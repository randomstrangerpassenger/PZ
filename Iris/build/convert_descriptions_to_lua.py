...
def build_usecase_chunk_index(
    descriptions_data: dict, chunks: list[tuple[int, str, int]]
) -> str:
    fulltypes = descriptions_data.get("fulltypes", {})
    sorted_fulltypes = sorted(fulltypes)
    offset = 0
    parts = [
        ...
        f"    entry_count = {len(sorted_fulltypes)},",
        "    chunks = {",
    ]
    for index, content, entry_count in chunks:
        keys = sorted_fulltypes[offset : offset + entry_count]
        offset += entry_count
        ...
        parts.append(
            ...
            % (
                ...
                lua_str(keys[0]),
                lua_str(keys[-1]),
                lua_str(f"Iris/Data/UseCaseDescriptions/Chunk{index:03d}"),
                lua_str(digest),
            )
        )
    parts.extend(["    },", "}", ""])
...
def build_usecase_line_count_index(descriptions_data: dict) -> str:
    fulltypes = descriptions_data.get("fulltypes", {})
    sorted_fulltypes = sorted(fulltypes)
    parts = [
        "-- Auto-generated by convert_descriptions_to_lua.py",
    ...
    for full_type in sorted_fulltypes:
        items = fulltypes[full_type].get("use_case_block", {}).get("items", [])
        line_count = sum(
            1
            for item in items
            if not str(item.get("use_case_id", "")).startswith("uc.exclusion.")
        )
        parts.append(f"        [{lua_str(full_type)}] = {line_count},")
    parts.extend(["    },", "}", ""])
    return "\n".join(parts)
