...
def create_external_evidence_root(name: str) -> Path:
    from clean_checkout_test_paths import external_test_path

    root = external_test_path(name)
    if root.exists():
        shutil.rmtree(root)
...
def residual_seal_environment(root: Path) -> dict[str, str]:
    environment = os.environ.copy()
    environment[EVIDENCE_ROOT_ENV] = str(root)
    return environment


def run_residual_seal(
    root: Path, *args: str
    ...
    return subprocess.run(
        ...
        capture_output=True,
        check=False,
        env=residual_seal_environment(root),
    )


def rebind_and_restore_complete_review(
    root: Path,
) -> subprocess.CompletedProcess[str]:
    decision = load_json(
    ...
    if decision.get("pending_author_selection") is True:
        return run_residual_seal(root, "--mode", "all")
    generated = run_residual_seal(root, "--mode", "generate")
    ...
    return run_residual_seal(root, "--mode", "all", "--require-complete")
