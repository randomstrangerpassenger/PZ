...
class PostCleanupPhase2RuntimeAdoptionTest(unittest.TestCase):
    def test_build_phase2_runtime_adoption_overlays_adopt_rows(self) -> None:
        ...
        try:
            ...
            facts = [
                json.loads(line)
                for line in (output_dir / "dvf_3_3_facts.adopted.jsonl").read_text(encoding="utf-8").splitlines()
                if line.strip()
            ]
            ...
            decisions = [
                json.loads(line)
                for line in (output_dir / "dvf_3_3_decisions.adopted.jsonl").read_text(encoding="utf-8").splitlines()
                if line.strip()
            ]
            ...
            rendered = json.loads(
                (output_dir / "dvf_3_3_rendered.adopted.json").read_text(encoding="utf-8")
            )
