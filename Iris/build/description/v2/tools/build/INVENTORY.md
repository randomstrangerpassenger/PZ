...
## Residual refactor Phase 0 readpoint (2026-08-03)

The earlier counts above remain historical snapshots. The residual-refactor
inventory is generated by
`Iris/validation/residual_refactor/report_inventory.py`; its authoritative
plan-local output is
`Iris/_docs/refactor/residual_refactor/phase0_inventory.json`.

| Metric (2026-08-03) | Result |
|---|---:|
| `tools/build` Python files, recursive | 496 |
| `tools/build` Python files, root-direct | 483 |
| Physical lines, including blank/comment | 240,005 |
| Nonblank lines | 220,214 |
| Git-tracked / untracked-or-ignored | 249 / 247 |
| `sys.path.insert/append` files | 151 |
| `argparse` files | 273 |
| AST-detected subprocess files | 51 |
| UTF-8 BOM files | 16 |
| Current core / allowed tooling | 12 / 4 |

Every file has one inventory role from `active`, `legacy-required`,
`completed-but-reproducible`, `historical`, `diagnostic`, `test-only`, or
`unknown`. An `unknown` role is an explicit preservation disposition: it is not
permission to move, delete, or commonize the file.

Change 5 keeps `compose_layer3_io.py` as the zero-new-stem in-place pilot. The
closure-external leaf pilot keeps `export_registry_runtime_records.py` as its
existing CLI/import adapter and extracts only stdlib path projection into
`registry_runtime_record_paths.py`. Neither file is added to the current core
12 or allowed tooling 4. `_dvf_3_3_vnext_common.py` remains a high-fan-in
compatibility fixture and is not an extraction target.
