...
def validate_authority_execution_inputs(
    ...
    *,
    ...
    blockers: list[str] = []
    o12_binding = load_json(o12_binding_path)
    if (
        o12_binding.get("status") != "PASS"
        or o12_binding.get("implementation_reentry_allowed") is not True
        or not all(o12_binding.get("predicates", {}).values())
    ):
        blockers.append("o12_phase_boundary_reconciliation_not_pass")
    ...
    *,
    ...
    write_json(
        no_relaxation_path,
        {
            ...
            "detector_source_commit": threshold_binding[
                "detector_source_commit"
            ],
            "detector_source_git_blob_id": threshold_binding[
                "detector_source_git_blob_id"
            ],
            "detector_symbol": "text_skeleton",
    ...
    threshold_predicates = {
        ...
        "canonical_text_skeleton_evaluation_deferred": canonical_deferment[
            "canonical_text_skeleton_evaluation_state"
        ]
        == "deferred_to_registry_adopted_fresh_naturalization_attempt",
        "canonical_text_skeleton_evaluation_count_zero": canonical_deferment[
            "canonical_text_skeleton_evaluation_count"
        ]
        == 0,
        "semantic_profile_as_text_skeleton_substitution_count_zero": (
            canonical_deferment[
                "semantic_profile_as_text_skeleton_substitution_count"
            ]
            == 0
        ),
        "maximum_same_skeleton_group_claim_emitted_count_zero": (
            canonical_deferment[
                "maximum_same_skeleton_group_claim_emitted_count"
            ]
            == 0
        ),
        "O-12_in_round_deferred_without_gate_credit": canonical_deferment[
            "O-12_in_round_disposition"
        ]
        == "deferred_without_gate_credit",
        "canonical_same_skeleton_acceptance_claim_allowed_false": (
            canonical_deferment[
                "canonical_same_skeleton_acceptance_claim_allowed"
            ]
            is False
        ),
    ...
    phase12_acceptance = {
        ...
        "canonical_text_skeleton_deferment_report_sha256": sha256_file(
            canonical_deferment_path
        ),
        "no_relaxation_attestation_sha256": sha256_file(
            no_relaxation_path
        ),
        "canonical_text_skeleton_evaluation_state": canonical_deferment[
            "canonical_text_skeleton_evaluation_state"
        ],
