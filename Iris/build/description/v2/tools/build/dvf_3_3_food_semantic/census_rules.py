...
O12_RECONCILIATION_DOC_PATH = (
    "docs/dvf_3_3_food_semantic_facts_authority_"
    "o12_phase_boundary_reconciliation.md"
)
O12_RECONCILIATION_CONTRACT_PATH = (
    "Iris/_docs/round3/dvf_3_3_food_semantic_facts_authority_reconstruction/"
    "o12_phase_boundary_reconciliation.json"
)
O12_RECONCILIATION_REVIEW_PATH = (
    "Iris/_docs/round3/dvf_3_3_food_semantic_facts_authority_reconstruction/"
    "o12_phase_boundary_reconciliation_review.json"
)
ATTEMPT_0021_EXTERNAL_REVIEW_PATH = (
    "Iris/build/description/v2/staging/"
    "dvf_3_3_food_semantic_facts_authority/attempts/attempt-0021/"
...
EXPECTED_O12_RECONCILIATION_DOC_SHA256 = (
    "996fd36e3b3ddfeec2b02e440ae78a335030dfe90fad9e7068bf52a75a788c56"
)
EXPECTED_O12_RECONCILIATION_CONTRACT_SHA256 = (
    "8f1b9e40141ee096bfa5ab17e4506aa67f759733a9c476023db6dbcbea5bf3c1"
)
EXPECTED_O12_RECONCILIATION_REVIEW_SHA256 = (
    "f5d70aecb2b92c46e167595392c28bec53cb8e958b71efd7a0e70cc2fadb82b3"
)
EXPECTED_ATTEMPT_0021_EXTERNAL_REVIEW_SHA256 = (
    "52dc1a43ad8ccf2e620bd5ca9cb0273c99a9015d03b915792fd09e0119b454c1"
)
...
def _plan_projection(snapshot: str) -> list[dict[str, Any]]:
    ...
    mappings = [
        ...
        (
            "O-12",
            "Changes 0,6,12",
            "canonical_threshold_detector_binding_and_downstream_deferment",
            [33, 34, 35],
            [
                "phase0_plan_and_decisions/"
                "o12_phase_boundary_reconciliation_binding.json",
                "phase12_phase2_handoff/threshold_authority_binding.json",
                "phase12_phase2_handoff/"
                "canonical_text_skeleton_deferment_report.json",
            ],
        ),
    ]
    rows = []
    for outcome, change, exit_name, line_numbers, evidence_paths in mappings:
        ...
        if outcome == "O-12":
            row["substantive_reconciliation_contract"] = (
                O12_RECONCILIATION_CONTRACT_PATH
            )
            row["in_round_disposition"] = "deferred_without_gate_credit"
            row["downstream_owner"] = (
                "registry-adopted fresh Naturalization attempt"
            )
        rows.append(row)
    return rows
...
def run_phase0(
    ...
    o12_doc_path = root / O12_RECONCILIATION_DOC_PATH
    o12_contract_path = root / O12_RECONCILIATION_CONTRACT_PATH
    o12_review_path = root / O12_RECONCILIATION_REVIEW_PATH
    attempt_0021_review_path = root / ATTEMPT_0021_EXTERNAL_REVIEW_PATH
    required_paths = [
        plan,
        predecessor,
        naturalization,
    ...
    o12_contract = load_json(o12_contract_path)
    o12_review = load_json(o12_review_path)
    ...
    o12_doc_sha256 = _normalized_text_sha256(o12_doc_path)
    o12_contract_sha256 = _normalized_text_sha256(o12_contract_path)
    o12_review_sha256 = _normalized_text_sha256(o12_review_path)
    attempt_0021_review_sha256 = _normalized_text_sha256(
        attempt_0021_review_path
    )
    o12_reentry = o12_review["implementation_reentry_gate"]
    o12_predicates = {
        "document_tracked": bool(
            _git(root, "ls-files", "--", O12_RECONCILIATION_DOC_PATH)
        ),
        "contract_tracked": bool(
            _git(root, "ls-files", "--", O12_RECONCILIATION_CONTRACT_PATH)
        ),
        "review_tracked": bool(
            _git(root, "ls-files", "--", O12_RECONCILIATION_REVIEW_PATH)
        ),
        "owner_ratified": o12_contract.get("status")
        == "owner_ratified_pending_independent_plan_review",
        "contract_id_match": o12_contract.get("contract_id")
        == "dvf3_3_food_semantic_o12_phase_boundary_reconciliation_0001",
        "document_identity_match": o12_doc_sha256
        == EXPECTED_O12_RECONCILIATION_DOC_SHA256
        == o12_reentry.get("required_reconciliation_markdown_sha256"),
        "contract_identity_match": o12_contract_sha256
        == EXPECTED_O12_RECONCILIATION_CONTRACT_SHA256
        == o12_reentry.get(
            "required_reconciliation_machine_contract_sha256"
        ),
        "review_identity_match": o12_review_sha256
        == EXPECTED_O12_RECONCILIATION_REVIEW_SHA256,
        "failed_attempt_review_identity_match": attempt_0021_review_sha256
        == EXPECTED_ATTEMPT_0021_EXTERNAL_REVIEW_SHA256
        == o12_reentry.get("required_failed_attempt_review_sha256"),
        "review_pass": o12_review.get("review_verdict") == "PASS",
        "open_critical_zero": o12_review["finding_counts"][
            "open_critical"
        ]
        == 0,
        "open_important_zero": o12_review["finding_counts"][
            "open_important"
        ]
        == 0,
        "review_gate_pass": o12_reentry.get("gate_result") == "PASS",
        "new_attempt_from_change0_required": (
            o12_reentry.get("new_attempt_required") is True
            and o12_reentry.get("new_attempt_entry") == "Change 0"
            and o12_reentry.get("same_attempt_correction_allowed") is False
        ),
        "facts_phase2_canonical_evaluation_deferred": o12_contract[
            "facts_phase2_contract"
        ]["canonical_text_skeleton_evaluation_state"]
        == "deferred_to_registry_adopted_fresh_naturalization_attempt",
        "facts_phase2_approximation_forbidden": o12_contract[
            "facts_phase2_contract"
        ]["semantic_profile_as_text_skeleton_substitution_count"]
        == 0,
        "o12_downstream_owner_bound": o12_contract["o12_disposition"][
            "downstream_owner"
        ]
        == "registry-adopted fresh Naturalization attempt",
        "o12_not_weakened": (
            o12_contract["o12_disposition"]["canonical_detector_required"]
            is True
            and o12_contract["o12_disposition"][
                "canonical_threshold_policy_required"
            ]
            is True
            and o12_contract["o12_disposition"][
                "detector_or_threshold_relaxation_allowed"
            ]
            is False
            and o12_contract["o12_disposition"]["waiver_allowed"] is False
        ),
    }
    o12_binding = {
        "schema_version": (
            "food-semantic-o12-phase-boundary-reconciliation-binding-v1"
        ),
        "status": "PASS" if all(o12_predicates.values()) else "BLOCKED",
        "document": asdict(identity(o12_doc_path, root=root)),
        "document_normalized_sha256": o12_doc_sha256,
        "machine_contract": asdict(identity(o12_contract_path, root=root)),
        "machine_contract_normalized_sha256": o12_contract_sha256,
        "independent_review": asdict(identity(o12_review_path, root=root)),
        "independent_review_normalized_sha256": o12_review_sha256,
        "failed_attempt_review": asdict(
            identity(attempt_0021_review_path, root=root)
        ),
        "failed_attempt_review_normalized_sha256": (
            attempt_0021_review_sha256
        ),
        "predicates": o12_predicates,
        "implementation_reentry_allowed": all(o12_predicates.values()),
        "terminal_independent_gate_credit": 0,
    }
    write_json(
        phase / "o12_phase_boundary_reconciliation_binding.json",
        o12_binding,
    )
    ...
    sync = {
        ...
        "o12_phase_boundary_reconciliation": {
            "contract_id": o12_contract["contract_id"],
            "binding_status": o12_binding["status"],
            "independent_review_verdict": o12_review["review_verdict"],
            "facts_phase2_canonical_text_skeleton_evaluation_state": (
                o12_contract["facts_phase2_contract"][
                    "canonical_text_skeleton_evaluation_state"
                ]
            ),
            "semantic_profile_as_text_skeleton_substitution_count": 0,
            "maximum_same_skeleton_group_claim_emitted_count": 0,
            "downstream_owner": o12_contract["o12_disposition"][
                "downstream_owner"
            ],
            "original_plan_bytes_mutated": False,
            "four_plan_projection_mutated": False,
        },
        "registry": {
    ...
    review_binding = {
        ...
        "substantive_o12_reconciliation_review": {
            "path": O12_RECONCILIATION_REVIEW_PATH,
            "normalized_sha256": o12_review_sha256,
            "verdict": o12_review["review_verdict"],
            "finding_counts": o12_review["finding_counts"],
        },
    ...
    predicates = {
        ...
        "o12_phase_boundary_reconciliation_binding": o12_binding["status"],
        "o12_reconciliation_independent_review": o12_review[
            "review_verdict"
        ],
        "o12_reconciliation_open_critical_count": o12_review[
            "finding_counts"
        ]["open_critical"],
        "o12_reconciliation_open_important_count": o12_review[
            "finding_counts"
        ]["open_important"],
        "o12_reconciliation_original_plan_mutation_count": 0,
        "o12_reconciliation_approximate_harness_allowed": False,
        "o12_reconciliation_new_attempt_entry": "Change 0",
    ...
    expected_false_predicates = {
        ...
        "o12_reconciliation_approximate_harness_allowed",
    }
...
def run_phase1(root: Path, attempt_root: Path) -> dict[str, Any]:
    ...
    protected_paths = [
        ...
        root / O12_RECONCILIATION_DOC_PATH,
        root / O12_RECONCILIATION_CONTRACT_PATH,
        root / O12_RECONCILIATION_REVIEW_PATH,
        root / ATTEMPT_0021_EXTERNAL_REVIEW_PATH,
    ]
    protected_trees = [
