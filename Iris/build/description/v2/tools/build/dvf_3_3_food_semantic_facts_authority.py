    ...
    results = _run_phases(
        [
            (
                "phase0",
                lambda: run_phase0(
                    root, attempt_root, g1_release_receipt_path
                ),
            ),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--g1-release-receipt",
        help=(
            "Path to the append-only G1 clean-validation downstream-unblock "
            "receipt; required by kernel and all."
        ),
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "kernel":
            if not args.g1_release_receipt:
                raise FoodSemanticError(
                    "--g1-release-receipt is required for kernel"
                )
            result = run_kernel(
                root,
                attempt_root,
                args.attempt_id,
                Path(args.g1_release_receipt),
            )
            ...
            if not args.g1_release_receipt:
                raise FoodSemanticError(
                    "--g1-release-receipt is required for all"
                )
            run_kernel(
                root,
                attempt_root,
