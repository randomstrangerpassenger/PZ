...
REPETITION_REMEDIATION_BASELINE_ATTEMPT_ID = "attempt-0011-handoff"
REPETITION_REMEDIATION_BASELINE_ROOT = (
    DEFAULT_ATTEMPT_PARENT / REPETITION_REMEDIATION_BASELINE_ATTEMPT_ID
)
...
def build_phase2(attempt_id: str, attempt_root: Path) -> dict[str, Any]:
    ...
    require_files((POLICY_PATH, BODY_PLAN_APPLICABILITY_APPROVAL_PATH))
    ...
    source_manifest = {
        "schema_version": "dvf-3-3-source-proposition-manifest-v1",
        "attempt_id": attempt_id,
        "source_path": repo_relative(FACTS_PATH),
    ...
    result = {
        "schema_version": "dvf-3-3-phase2-result-v1",
        "attempt_id": attempt_id,
        "status": "PASS",
...
def build_phase3_repetition_remediation_reports(
    *,
    ...
    require_files((baseline_hit_path, baseline_report_path))
    baseline_report = load_json(baseline_report_path)
    baseline_hits = [
        row
        for row in load_jsonl(baseline_hit_path)
        if row.get("detector_id") == "repeated_skeleton_concentration"
        and row.get("hit") is True
    ]
    if (
        baseline_report.get("candidate_rendered_hash")
        != "e3cc76d2e9714d3c1d9164889fa524a6d24539228459ec024e4b041406db00b7"
        or len(baseline_hits) != 450
    ):
        raise NaturalizationError("preserved repetition-remediation baseline drift")
    facts_rows = load_jsonl(FACTS_PATH)
    decisions_rows = load_jsonl(DECISIONS_PATH)
    ...
    baseline_hit_ids = {str(row["item_id"]) for row in baseline_hits}
    ...
    cause_report = {
        "schema_version": "dvf-3-3-repeated-skeleton-cause-analysis-v2",
        "attempt_id": attempt_id,
        "preserved_baseline_attempt_id": REPETITION_REMEDIATION_BASELINE_ATTEMPT_ID,
        "preserved_baseline_raw_detector_report_path": repo_relative(
            baseline_report_path
        ),
        "preserved_baseline_raw_detector_report_sha256": sha256_file(
            baseline_report_path
        ),
        "preserved_baseline_hit_ledger_sha256": sha256_file(baseline_hit_path),
...
def build_phase3(attempt_id: str, attempt_root: Path) -> dict[str, Any]:
    require_phase0(attempt_root)
    require_files((phase_root(attempt_root, 2) / "phase2_result.json",))
...
def build_phase7(attempt_id: str, attempt_root: Path) -> dict[str, Any]:
    ...
    manifest = {
        "schema_version": "dvf-3-3-human-review-sample-manifest-v1",
        "naturalization_attempt_id": attempt_id,
...
def build_phase8_handoff(attempt_id: str, attempt_root: Path) -> dict[str, Any]:
    ...
    if not blockers:
        ...
        write_once_or_same(root / "publish_acceptance_handoff_manifest.json", handoff)
    return readiness
