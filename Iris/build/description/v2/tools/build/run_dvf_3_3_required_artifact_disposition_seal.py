...
from dvf_3_3_required_artifact_disposition_seal_common import (
    generate_artifacts,
    validate_artifacts,
)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run DVF 3-3 required artifact disposition seal.")
    parser.add_argument(
        "--mode",
    ...
    if args.mode in {"census", "standard", "all", "machine-pass"}:
            ...
            json.dumps(
                {
                    ...
                    "required_artifact_disposition_problem_status": final.get(
                        "required_artifact_disposition_problem_status"
                    ),
                    "machine_pass_blocked": final.get("machine_pass_blocked"),
        ...
        if args.mode == "census":
            return 0 if final.get("terminal_state") in {"ready", "owner_pending", "complete_with_blockers"} else 1
