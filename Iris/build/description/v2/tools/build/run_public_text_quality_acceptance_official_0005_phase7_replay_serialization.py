...
import public_text_quality_acceptance_official_0005_phase7_replay_serialization as phase7
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--mode", required=True, choices=("current-verification", "freeze", "finalize", "handoff"))
    args = parser.parse_args(argv)
    if args.attempt_id != "attempt-0005-official":
        print(json.dumps({"status": "FAIL", "error": "attempt ID mismatch"}), file=sys.stderr)
        return 2
    try:
        if args.mode == "current-verification":
            result = phase7.materialize_current_verification()
