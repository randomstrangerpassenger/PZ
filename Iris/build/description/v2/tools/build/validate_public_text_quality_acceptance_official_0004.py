...
from public_text_quality_acceptance_official_0004 import (
    ATTEMPT_ID,
...
def main(argv: list[str] | None = None) -> int:
    ...
    selected = [
        ...
        if getattr(args, option.removeprefix("--").replace("-", "_"))
    ]
    if args.attempt_id != ATTEMPT_ID or len(selected) != 1:
        print(
            json.dumps(
                {"status": "FAIL", "error": "exactly one official-0004 requirement is required"}
            ),
            file=sys.stderr,
        )
        return 2
    try:
        ...
        print(
            json.dumps({"status": "FAIL", "error": str(exc)}, ensure_ascii=False, sort_keys=True),
            file=sys.stderr,
        )
        return 2
