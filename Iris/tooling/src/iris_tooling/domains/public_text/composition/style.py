...
from iris_tooling.common.repository_context import require_repository_context


def _default_rules_dir() -> Path:
    require_repository_context()
    # The implementation moved; existing packaged rule data keeps its path.
    return Path(__file__).resolve().parents[3] / "build" / "style_rules"
...
def _postprocess_ko(text: str) -> str:
...
def _matches_selected_cluster_contains(
...
class StyleNormalizer:
    def __init__(
        ...
        *,
        ...
        self.global_rules_doc = _load_json(self.rules_dir / "global_rules.json")
        self.family_rules_doc = _load_json(self.rules_dir / "family_rules.json")
    ...
    def _iter_matching_family_rules(
        ...
        *,
        ...
        for rule in self.family_rules_doc.get("rules", []):
            ...
            if (
                condition.get("fact_origin")
                and condition.get("fact_origin") != fact_origin
            ):
                continue
            cluster_condition = condition.get("selected_cluster_contains")
            if cluster_condition and not _matches_selected_cluster_contains(
                selected_cluster,
                cluster_condition,
            ):
                continue
    ...
    @staticmethod
    def _apply_rule(text: str, rule: dict[str, Any]) -> tuple[str, bool]:
        match_type = rule.get("type", "literal")
        match = rule["match"]
        replacement = rule["replace"]
        if match_type == "literal":
            updated = text.replace(match, replacement)
        elif match_type == "regex":
            updated = re.sub(match, replacement, text)
        else:
        ...
        *,
        ...
        if not manual_override:
            ...
            for rule in self._iter_matching_family_rules(
                ...
                if changed:
                    applied_rules.append(rule["id"])
        normalized = _postprocess_ko(working)
