...
def _base_identity(entry: dict) -> str:
    """Compact runtime stale-pair guard for the exact bilingual base rows."""
    hashes = [17, 29]
    moduli = [2147483647, 2147483629]

    def update(byte: int) -> None:
        for index, modulus in enumerate(moduli):
            hashes[index] = (hashes[index] * 257 + byte + 1) % modulus

    for marker, locale in ((1, "ko"), (2, "en")):
        update(marker)
        for row in entry[locale]:
            for byte in row.encode("utf-8"):
                update(byte)
            update(0)
        update(255)
    return f"{hashes[0]}:{hashes[1]}:{len(entry['ko'])}:{len(entry['en'])}"
...
def variants_bytes(data: dict) -> bytes:
    def arrays(entry, indent, key):
        rendered = []
        for loc in ("ko", "en"):
            values = []
            for index, value in enumerate(entry[loc], 1):
                if index <= len(data[key]["base"][loc]) and value == data[key]["base"][loc][index - 1]:
                    values.append(f"baseRow({lua_string(key)}, {lua_string(loc)}, {index})")
                else:
                    values.append(lua_string(value))
            rendered.append(f"{indent}{loc} = {{" + ", ".join(values) + "},")
        return rendered
    ...
    for key in sorted(data):
        lines.append(f"bases[{lua_string(key)}] = static[{lua_string(key)}]")
    lines.extend([
        "",
    ...
    for key in sorted(data):
        ...
        lines.extend([
            f"    [{lua_string(key)}] = {{",
            f"        base = bases[{lua_string(key)}],",
            f"        base_identity = {lua_string(_base_identity(entry['base']))},",
            "        variants = {",
        ])
        for variant in entry["variants"]:
            ...
            lines.extend(arrays(variant, "                ", key))
            lines.append("            },")
        lines.append("        },")
        if "without_recipe" in entry:
            lines.append("        without_recipe = {")
            lines.extend(arrays(entry["without_recipe"], "            ", key))
            lines.append("        },")
        lines.append("    },")
    lines.append("}")
    return ("\n".join(lines) + "\n").encode("utf-8")
