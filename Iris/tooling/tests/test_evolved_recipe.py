...
def test_source_accounting_semantics_and_locale_review_isolation(
    ...
    assert water_pot[0]["display_by_locale"] == {
        "EN": "Can be used to prepare soup",
        "KO": "수프 준비에 사용할 수 있음",
    }
...
def test_canonical_owner_includes_all_definition_base_relations() -> None:
    ...
    for relations in owner["relations_by_fulltype"].values():
        for relation in relations:
            assert relation["display_by_locale"]["EN"].startswith("Can ")
    ...
    assert salt_beer["display_by_locale"] == {
        "KO": "텀블러에 담긴 맥주에 양념으로 넣을 수 있음",
        "EN": "Can be added to beer in a tumbler as seasoning",
    }
...
def test_candidate_projection_is_deterministic_and_owner_traced(
    ...
    assert manifest_a["schema_version"] == "iris-layer4-evolved-recipe-candidate-v6"
...
"""
    ...
    assert runtime_result.stdout.split(b"\x1f") == [
        "수프에 재료로 넣을 수 있음".encode("utf-8"),
        "스튜에 재료로 넣으려면 먼저 익혀야 함".encode("utf-8"),
        "수프에 양념으로 넣을 수 있음".encode("utf-8"),
        "수프 준비에 사용할 수 있음".encode("utf-8"),
    ]
