...
from promote_lifecycle_evidence import (
    PromotionError,
    validate_baseline_promotion_payload,
    validate_chain as validate_archive_chain,
)
from inventory_artifact_lifecycle import (
    SCOPED_ROOTS,
    LifecycleError,
    build_rows,
    git_path_set,
...
def validate_pre_delete_receipt(repo: Path, receipt_path: Path) -> dict[str, Any]:
    ...
    for key, relative_input in (
        ...
        ("required_validations", "Iris/validation/execution/required_validations.json"),
    ...
    for binding in command_rows:
        ...
        if command_id.endswith("pre-delete-current-route"):
            ...
            runner = validation_root / "Iris/validation/execution/run_required_contract_tests.py"
            if (
            ...
            runner_blob = subprocess.run(
                ["git", "-C", str(validation_root), "rev-parse", f"{subject['commit']}:Iris/validation/execution/run_required_contract_tests.py"],
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                text=True,
                encoding="utf-8",
        ...
        if command_id.endswith("verify-current-route-output-isolation"):
            ...
            audit_script = (
                validation_root
                / "Iris/validation/execution/audit_test_output_isolation.py"
            )
            if (
                not same_path(command.get("executable"), Path(sys.executable))
                or len(argv) != 11
            ...
            audit_blob = subprocess.run(
                [
                    ...
                    str(validation_root),
                    "rev-parse",
                    f"{subject['commit']}:Iris/validation/execution/audit_test_output_isolation.py",
                ],
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                text=True,
