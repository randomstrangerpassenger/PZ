...
LIFECYCLE_TRACKING_ADDITIONS.update(
    {
        ...
        "Iris/validation/artifacts/lifecycle_delta_codec.py",
        "Iris/validation/artifacts/migrate_evidence_storage.py",
...
def current_required_source_paths(repo: Path) -> set[str]:
    ...
    required_path = repo / "Iris/validation/execution/required_validations.json"
    ...
    for relative in source_paths:
        ...
        for basename in hits:
            for target in by_basename[basename]:
                ...
                elif relative.startswith("Iris/validation/artifacts/"):
...
def summary_for(repo: Path, subject_kind: str, identity: dict[str, Any], rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    inventory_producer_path = repo / "Iris/validation/source_analysis/inventory_build_tool_dependencies.py"
    required_validation_path = repo / "Iris/validation/execution/required_validations.json"
