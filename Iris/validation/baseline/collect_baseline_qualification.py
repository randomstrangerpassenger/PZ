...
from Iris.validation.baseline.qualification_contracts import (
...
def _build_rtc_forensic_observation(
    ...
    required_manifest_path = repo / "Iris/validation/execution/required_validations.json"
...
def forensic(repo: Path, result_root: Path) -> dict:
    ...
    commands = {
        ...
        "exact_current": [
            sys.executable, "-B", "Iris/validation/execution/run_required_contract_tests.py",
        ...
        "rtc_required_gate": [
            ...
            "--required-gate",
            "--required-manifest", "Iris/validation/execution/required_validations.json",
...
def _validate_rtc_bundle(repo: Path, subject: dict[str, str]) -> dict[str, object]:
    required = read_json(repo / "Iris/validation/execution/required_validations.json")
    selection = required.get("registry_runtime_compatibility")
    if not isinstance(selection, dict):
...
def _finalize_qualification_manifest(
    *,
    ...
    preconditions = read_json(repo / "Iris/validation/baseline/contracts/admission_precondition_registry.json")
    fixtures = read_json(repo / "Iris/validation/baseline/contracts/admission_negative_fixture_registry.json")
...
def qualify(args: argparse.Namespace) -> dict:
    ...
    launcher_relative = Path("Iris/validation/execution/invoke_repository_tests.ps1")
    comparator_relative = Path("Iris/validation/execution/compare_repeated_test_runs.ps1")
    ...
    path_contract_path = repo / "Iris/validation/baseline/contracts/windows_path_contract.json"
