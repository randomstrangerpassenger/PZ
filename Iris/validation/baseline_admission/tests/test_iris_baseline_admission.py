...
from Iris.validation.baseline_admission.iris_baseline_admission_common import path_preflight, read_json, sha256_file, validate_registry
from Iris.validation.baseline_admission.run_iris_baseline_admission import (
    _classify_observed_failure,
    _windows_powershell_environment,
    forensic_evidence_capture_pass,
)
from Iris.validation.baseline_admission.validate_iris_baseline_admission import execute_negative_matrix, synthetic_gate
...
def test_full_gate_declares_dedicated_route_disposition() -> None:
    contract = json.loads((REPO / "Iris/validation/clean_checkout/contracts/full_repository_gate.json").read_text(encoding="utf-8"))
