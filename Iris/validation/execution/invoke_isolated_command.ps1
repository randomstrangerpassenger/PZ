...
try {
    ...
    $expectedWrapper = [System.IO.Path]::GetFullPath((Join-Path $repositoryRoot 'Iris\validation\execution\invoke_isolated_command.ps1'))
    ...
    $authorityPath = Join-Path $repositoryRoot 'Iris\validation\execution\current_environment.json'
    $successorPolicyPath = Join-Path $repositoryRoot 'Iris\validation\execution\contracts\isolated_command_output_policy.json'
    ...
    foreach ($row in @(
        @('wrapper', 'Iris/validation/execution/invoke_isolated_command.ps1', $actualWrapper),
        @('successor_policy', 'Iris/validation/execution/contracts/isolated_command_output_policy.json', $successorPolicyPath),
        @('environment_authority', 'Iris/validation/execution/current_environment.json', $authorityPath)
    )) {
