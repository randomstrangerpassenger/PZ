"""Inventory and execute the tests required for an exact repository revision.

The source-census command only reads sources and dependency declarations. The
gate and full-gate commands execute the selected tests in isolated checkouts and
write their results to caller-supplied directories outside the source checkout.
"""
...
from Iris.validation.execution import (
    checkout_environment as clean_checkout_common,
    checkout_workspace,
    process_results,
    repository_contracts,
)
from Iris.validation.source_analysis import repository_sources
...
from Iris.validation.execution.repository_contracts import (
    ...
    _validate_current_capsule_required_evidence,
    _validate_g5_required_evidence,
    _line_ending_sha256_variants,
    _normalized_compiler_rows,
    _compiler_aggregate_sha256,
    _git_is_ancestor,
    _git_commit_is_available,
)
from Iris.validation.source_analysis.repository_sources import (
    _test_sources,
    _parse_pytest_ini,
    _under_configured_root,
    _taxonomy_indexes,
    _required_test_ids,
    _canonical_gate_sources,
    _full_required_pytest_sources,
    _explicit_current_required_paths,
    ...
    _imports,
    _module_candidates,
    _resolve_repository_module,
    _required_dependency_paths,
    _validate_explicit_tool_dispositions,
    _active_ignored_paths,
    build_source_census,
)
from Iris.validation.execution.checkout_workspace import (
    _materialize_menu_inputs,
    _require_disjoint_external_roots,
    _require_empty_directory,
    _safe_checkout_target,
    _materialize_frozen_predecessor_fixture,
    _materialize_package_runtime_mirror,
    _remove_disposable_checkout,
    _materialize_current_output_seed,
    _publish_current_output_seed,
    _clone_current_output_seed,
)
from Iris.validation.execution.process_results import (
    _run_subprocess,
...
def _implementation_identity(
    ...
    # Extracted implementation must belong to the same immutable subject as
    # the entry point. Keep the existing result schema and source-census owner.
    for module in (checkout_workspace, process_results, repository_contracts, repository_sources):
        logical_path = module.__name__.replace('.', '/') + '.py'
        actual_path = Path(module.__file__).resolve()
        if actual_path != (repo / logical_path).resolve():
            raise CleanCheckoutError(f"runner dependency imported from a different checkout: {logical_path}")
        expected_blob = blob_id(repo, commit, logical_path)
        working_blob = git_text(repo, 'hash-object', f'--path={logical_path}', str(actual_path)).strip()
        if working_blob != expected_blob:
            raise CleanCheckoutError(f"runner dependency working file differs from subject blob: {logical_path}")
...
def run_full_repository_gate(
    ...
    try:
        ...
        _materialize_menu_inputs(repo, checkout, contract, environment, result_root)
