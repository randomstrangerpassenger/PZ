...
WRAPPER = REPO / "Iris/validation/execution/invoke_isolated_command.ps1"
...
def fixture_checkout(tmp_path: Path) -> tuple[Path, Path, Path, Path]:
    ...
    contracts = repo / "Iris/validation/execution/contracts"
    ...
    write_json(
        repo / "Iris/validation/execution/current_environment.json",
        {
    ...
    write_json(
        contracts / "isolated_command_output_policy.json",
        {
    ...
    *,
    ...
    completed = subprocess.run(
        [
            ...
            "Bypass",
            "-File",
            str(repo / "Iris/validation/execution/invoke_isolated_command.ps1"),
            "-CommandSpec",
            str(spec),
        ],
        cwd=repo,
...
def test_scalar_command_spec_preserves_windows_argv_and_streams(tmp_path: Path) -> None:
    ...
    assert receipt["successor_policy"]["sha256"] == sha256(
        repo / "Iris/validation/execution/contracts/isolated_command_output_policy.json"
    )
...
def test_json_scalar_argv_is_rejected_without_execution(tmp_path: Path) -> None:
    ...
    completed = subprocess.run(
        [powershell(), "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", str(repo / "Iris/validation/execution/invoke_isolated_command.ps1"), "-CommandSpec", str(spec)],
        cwd=repo,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        text=True,
