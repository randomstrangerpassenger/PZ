...
from Iris.validation.source_analysis.inventory_offline_tooling_at_commit import (
    build_inventory,
)
from Iris.validation.execution.run_repository_tests import (
...
from Iris.validation.execution.validate_environment_and_results import (
...
def _build_fake_launcher_repository(
    ...
    *,
    ...
    execution_root = repo / "Iris/validation/execution"
    execution_root.mkdir(parents=True)
    contracts = execution_root / "contracts"
    authority = clean_checkout_root / "authority"
    contracts.mkdir(parents=True)
    authority.mkdir(parents=True)
    fixture_json_paths = (
        "Iris/validation/execution/contracts/evidence_storage_output_policy.json",
    ...
    allocator_fixture = (
        execution_root
        / "allocate_external_workspaces.ps1"
    )
    allocator_fixture.write_text("# fixture\n", encoding="utf-8", newline="\n")
    launcher_source = Path(__file__).resolve().parents[1] / (
        "invoke_repository_tests.ps1"
    )
    (execution_root / launcher_source.name).write_bytes(
        launcher_source.read_bytes()
    )
    compare_source = Path(__file__).resolve().parents[1] / (
        "compare_repeated_test_runs.ps1"
    )
    (execution_root / compare_source.name).write_bytes(
        compare_source.read_bytes()
    )
    (execution_root / "checkout_environment.py").write_text(
        "FAKE_COMMON_MARKER = 'actual-import-fixture'\n",
        encoding="utf-8",
    )
    runner = execution_root / "run_repository_tests.py"
    runner.write_text(
        textwrap.dedent(
            r'''
            from __future__ import annotations
            ...
            import checkout_environment as imported_common
    ...
    validator = (
        execution_root / "validate_environment_and_results.py"
    )
    validator.write_text(
        textwrap.dedent(
            ...
            common_path = Path(imported_common.__file__).resolve()
            validator_relative = (
                "Iris/validation/execution/validate_environment_and_results.py"
            )
            common_relative = (
                "Iris/validation/execution/checkout_environment.py"
            )
    ...
    (contracts / "test_execution_output_policy.json").write_bytes(
        canonical_json_bytes(
            {
    ...
    (contracts / "isolated_command_output_policy.json").write_bytes(
        canonical_json_bytes(
            {
                "schema_version": (
    ...
    (execution_root / "current_environment.json").write_bytes(
        canonical_json_bytes(
            {
...
def test_full_source_policy_classifies_only_declared_fallback(
    ...
    dedicated_sources = {
        (
            "Iris/validation/scenarios/tests/"
            "test_public_text_scenario_reuse.py"
        ),
        (
            "Iris/validation/scenarios/tests/"
...
def test_ignored_status_snapshot_excludes_nonignored_rows(
    ...
    monkeypatch.setattr(
        "Iris.validation.execution.run_repository_tests."
        "_status_snapshot",
...
def test_offline_tool_inventory_keeps_denominator_axes_and_registry_bindings(
    ...
    schema_blob = _git_bytes(
        ...
        "show",
        (
            f"{commit}:Iris/validation/execution/contracts/"
            "output_isolation_batch_registry.schema.json"
        ),
    )
