...
Standalone Lua fixture는 operation count와 output parity만 승인한다. 실제 Project Zomboid sample이 없는 debounce, incremental build, Tooltip first-load 및 LineCount attribution은 architecture adoption이 아니라 deferred candidate이며, 이 경계의 closeout 상태는 `partial`이다.
