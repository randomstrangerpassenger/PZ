...
### Repository validation 경계

Iris의 repository validation은 제품 런타임이나 정보 생산 계층이 아니라, tracked repository state가 선언된 검증 계약을 재현할 수 있는지 확인하는 오프라인 경계다.

```text
tracked repository state
+ declared validation environment
-> test/input role classification
-> deterministic bootstrap
-> isolated validation execution
-> reproducibility evidence
```

이 경계는 필수 검증 입력의 역할, commit-bound test inventory, deterministic materialization과 실행 출력 격리를 소유한다. 반면 source fact 작성, semantic 판정, rendered body 생성, artifact current adoption, public text acceptance와 package/release 판단은 소유하지 않는다.

따라서 repository validation 결과는 각 authority의 산출물을 검증 입력으로 소비할 수 있지만, 해당 authority의 의미·채택·출판 결정을 대신하지 않는다. 반대로 Food Semantic Facts Authority, DVF Body Compiler, Artifact Registry, Naturalization, Publish Boundary도 로컬 잔존 파일이나 과거 실행 상태를 repository validation authority로 사용할 수 없다.
