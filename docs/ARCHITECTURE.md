...
> 상태: 초안 v0.7
> 기준일: 2026-09-16 (이번 갱신 범위: Iris offline·runner·Lua 책임 분리, optional product pointer 처리, runtime/build/storage 경량화와 사용자 인게임 수락; 기존 DVF·B41/B42·제품 current 경계 유지)
...
### Runtime/build 경량화 경계 (2026-09-16)

상태는 **complete**다. 계획된 source/runtime 구현과 focused·Recovery·B→Menu·Lua syntax 계약이 통과했고 사용자가 최종 저장소 Iris의 실제 PZ 검증 통과를 보고했다. 이 완료는 아래 표현·UI 경량화 subject에 한정된다.

EvolvedRecipe runtime은 relation record를 유지하면서 동일 condition 배열과 bilingual label/action/display 값을 generated module 안에서 공유한다. 공유 값은 내부 read-only이며 Browser interaction projection이 UI 소유 condition 배열을 만든다. Tooltip variant module은 StaticData base를 참조하고 base와 같은 행을 재사용하되 완성된 KO/EN view 배열을 offline에서 계속 확정한다. lookup은 StaticData table reference와 bilingual identity를 함께 확인해 서로 다른 두 파일의 stale pair를 거부한다. 두 파일 install/package 경계는 유지한다.

경량화된 data 흐름은 `owner → deterministic shared-value pool → generated lookup → read-only lookup → consumer-owned projection`이다. Tooltip은 `StaticData + RecipeVariants reference → bilingual identity 검사 → opening 단위 final view 선택`이며 runtime prefix/delta 의미 합성이나 새 public 불변성 API를 추가하지 않는다. Generated bytes 감소는 architecture 결과지만 PZ heap·latency·FPS 개선은 측정하지 않았으므로 비주장이다.

Browser Detail은 identity, Layer 3, literature, interaction, variants, metadata section의 child와 base position을 추적한다. Layer 3 target group, interaction 검색·펼침, variant 펼침은 해당 section만 교체하고 뒤 section 위치와 scroll 범위를 조정한다. item, locale, Browser generation, width 변경은 기존 full rebuild 경계이며 정적 detail model을 다시 쓰지 않는다. Recovery는 read-only fact/provenance view를 공유하고 실제로 필터링하는 application 및 authority가 바뀌는 acquisition row만 복사한다.

정확한 측정·검사와 사용자의 실제 PZ 통과 보고는 [경량화 closeout](iris_runtime_build_storage_optimization_closeout.md)이 소유한다. 안내된 KO/EN Tooltip·Menu 공존과 Browser 부분 갱신 관찰 범위까지 완료했으며, 게임 빌드·해상도·UI 배율·외부 모드 구성은 별도로 제공되지 않았다. 이 구조 변경은 새 runtime 의미 추론, 새 validation authority, current-product adoption, release를 추가하지 않는다.
