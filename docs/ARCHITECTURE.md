...
## 11-65. Iris refactor v2.0 closes runtime/build boundary debt without changing product authority

2026-05-08 기준 Iris refactor roadmap v2.0은 implementation-complete 상태다. 이 closeout은 product authority를 새로 여는 round가 아니라, already-adopted Iris runtime/build contracts를 더 명시적인 파일 경계로 수렴시키는 conservative refactor다.

### current runtime shape

- protected-call boundary:
  - owner: `Iris/Util/IrisProtectedCall.lua`
  - all direct runtime `pcall` usage under `media/lua/client` routes through this boundary
  - `IrisRequire.safeRequire` delegates to `IrisProtectedCall.require`
- use-case generated data:
  - public module: `Iris/Data/IrisUseCaseDescriptions`
  - facade file: `Iris/Data/IrisUseCaseDescriptions.lua`
  - payload files: `Iris/Data/UseCaseDescriptions/Chunk001..009.lua`
  - runtime JSON parser: not introduced
- description implementation:
  - current namespace: `Iris/Logic/IrisDesc/*`
  - legacy namespace: `Pulse/Iris/Logic/IrisDesc/*` wrappers only
- browser interaction display:
  - requirement color/text policy owner: `Iris/UI/Browser/IrisRequirementPolicy.lua`
  - MapIcon positioning owner: `IrisConfig.MAP_ICON_BUTTON`
- build surface:
  - root `Iris/build` Python scripts are documented entrypoints
  - pipeline helpers, one-shots, tests, and versioned policy data live under explicit subdirectories

### preserved contracts

- Iris remains 100% Lua at runtime.
- Pulse does not depend on Iris.
- Iris does not depend on other spokes.
- Existing consumer require path `Iris/Data/IrisUseCaseDescriptions` remains stable.
- Compatibility wrappers keep old `Pulse/Iris/Logic/IrisDesc/*` consumers from breaking during transition.
- The refactor does not change DVF 3-3 publish/quality/runtime authority, source expansion status, or release readiness.

### validation read

- `319 tests / OK` for `Iris\build\description\v2\tests`
- `test_require_render.py`: PASS
- `quality_gates.py`: PASS
- `package_iris.ps1 -Clean`: PASS

This section supersedes any local note that described deferred refactor items as policy-only closure. Current architecture reads them as implemented code.

## 11-66. Iris runtime Lua files must be BOM-free for PZ Kahlua compatibility

2026-05-08 item-selection regression 확인 후, Iris runtime Lua file encoding is part of architecture correctness.

### regression

After moving `IrisDesc` implementation files to `Iris/Logic/IrisDesc/*`, those files retained UTF-8 BOM. PZ Kahlua compiler failed while requiring `Iris/Logic/IrisDesc/Generator.lua`, and item selection retried the generator load, causing one error per selection.

### adopted guard

- `Iris/Logic/IrisDesc/*.lua` implementation files are BOM-free.
- `IrisAPI.lua` has `IrisDescGeneratorLoadAttempted` to prevent repeated require attempts after a generator load failure.
- Targeted in-game smoke confirms the item-selection error accumulation is fixed.

### boundary

This is targeted runtime compatibility smoke, not full release QA. It does not declare deployed closeout, external-mod expansion readiness, or `ready_for_release`.

## 11-67. Iris final refactoring roadmap closes the runtime/build structure, not the release

2026-05-08 기준 `docs/Iris/iris-refactoring-final-roadmap-v1.md`는 implementation closeout 상태다. 이 closeout의 authority는 `Iris/_docs/refactor/iris_refactoring_final_roadmap_closeout.md`다.

### closeout read

- scope: Phase 1 through Phase 5-9
- final roadmap item: Phase 5-9 module bootstrap boilerplate consolidation
- static validation: `376 tests / OK`
- runtime validation: KO console smoke
- package/release state: not declared

### current module bootstrap shape

Runtime modules that need shared `safeRequire` and logger fallback setup read through:

- `Iris/Util/IrisModuleBootstrap.lua`

The helper owns:

- `safeRequire` access through `Iris/Util/IrisRequire`
- optional `Iris/Util/IrisLogger` resolution
- no-op logger fallback for ordinary runtime modules
- print-backed fallback for `IrisMain.lua` boot visibility

`IrisLogger.lua` does not require `IrisModuleBootstrap.lua`; this avoids a logger/bootstrap require cycle.

### current runtime smoke read

Latest KO console validation confirms:

- `Iris/Util/IrisModuleBootstrap.lua` loads during Iris boot
- `Iris/Util/IrisRequire.lua`, `Iris/Util/IrisLogger.lua`, and `Iris/IrisConfig.lua` load after the bootstrap helper
- `IrisMain loaded successfully`
- `[Iris] Bootstrap complete`
- `TestHarness` does not load in production
- Iris Lua error patterns are 0

### boundary

This section declares implementation, static-test, and console-validation closeout for the refactoring roadmap. It does not declare packaging completion, git commit completion, release note completion, full release manual QA, Workshop deployment, external mod expansion readiness, or `ready_for_release`.

## 11-68. Iris refactoring roadmap v4.1 seals workspace-copy runtime structure

2026-05-12 기준 `C:/Users/MW/Downloads/1.txt`의 `최종 리팩토링 로드맵 v4.1`은 implementation, static-test, and workspace-copy in-game console-validation level에서 닫혔다. 이 section은 11-67의 final refactoring closeout을 current validation path 기준으로 보강한다.

### current runtime data authority

Layer 3 runtime data is chunks-only for deployable Iris runtime layout.

- manifest: `Iris/Data/IrisLayer3DataChunks.lua`
- chunk files: `Iris/Data/IrisLayer3DataChunks/Chunk001..011.lua`
- renderer entry: `Iris/Data/layer3_renderer.lua`
- monolith path: `Iris/Data/IrisLayer3Data.lua` is not active deployable runtime authority

`layer3_renderer.lua` must not reopen `safeRequire("Iris/Data/IrisLayer3Data")` as a normal fallback. If runtime data fails to load, the valid first fix is the chunk manifest/chunk deployment contract, not restoring monolith/chunk dual loading.

### workspace-copy validation contract

The user validation path deletes `C:/Users/MW/Zomboid/mods/Iris` and copies the workspace `Iris` folder directly into the PZ mods directory. Therefore packaging-only exclusion is insufficient architecture protection. Active workspace layout itself must avoid shipping the Layer 3 monolith under `media/lua/client`.

Latest console validation reads:

- source: `C:/Users/MW/Zomboid/Console.txt`
- last write time: `2026-05-12 20:42:50`
...
### runtime helper structure sealed by v4.1

The v4.1 refactor adds or confirms the following runtime helper boundaries:

- Browser chrome/theme constants: `Iris/UI/Browser/IrisBrowserTheme.lua`
- Translation fallback resolution: `Iris/Util/IrisTranslationResolver.lua`
- Generic Java/Kahlua object call access: `Iris/Util/IrisObjectAccess.lua`
- Item-specific key/display/type access: `Iris/Util/IrisItemAccess.lua`
- Use-case requirements lookup: `Iris/Data/UseCaseDescriptions/RequirementsLookup.lua`
- Context-menu texture compatibility guard: `Iris/Compat/IrisContextMenuTextureCompat.lua`

These helpers narrow responsibility boundaries without changing Iris product authority. They do not introduce Pulse dependency, other-spoke dependency, runtime JSON parsing, or external mod integration.

### validation read

- `python -B -m unittest discover -s Iris\build\description\v2\tests -p "test_*.py"`: `380 tests / OK`
- `python -B Iris\build\test_require_render.py`: PASS
- `python -B Iris\build\quality_gates.py`: PASS
- in-game console context-menu regression markers: 0 matches

### boundary

This section does not declare P1 build script manifest completion, P2 protected-call policy completion, packaging completion, git commit completion, release notes, Workshop deployment, full release manual QA, external mod expansion readiness, or `ready_for_release`.
