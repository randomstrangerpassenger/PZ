...
# 31. 2026-05-08 Addendum — Iris refactor v2.0 implementation closeout and item-selection regression fix

## 이번 addendum의 상태 해석

- 이번 addendum은 Iris refactor roadmap v2.0이 policy-only close가 아니라 code implementation으로 닫혔음을 기록한다.
- closeout state는 `implemented_and_validated`다.
- item selection마다 error가 누적되던 regression은 targeted in-game smoke 기준 fixed다.
- 이 addendum은 full release QA, deployed closeout, `ready_for_release`를 선언하지 않는다.

## Iris

### Done

- protected-call boundary를 구현했다.
  - `Iris/Util/IrisProtectedCall.lua`
  - direct runtime `pcall`은 이 boundary로 수렴
  - `IrisRequire.safeRequire`는 `IrisProtectedCall.require`를 사용
- generated use-case data를 Lua facade/chunk 구조로 분리했다.
  - public facade: `Iris/Data/IrisUseCaseDescriptions.lua`
  - chunks: `Iris/Data/UseCaseDescriptions/Chunk001..009.lua`
  - runtime JSON parser는 도입하지 않음
- `IrisDesc` implementation namespace를 정리했다.
  - current implementation: `Iris/Logic/IrisDesc/*`
  - legacy compatibility wrapper: `Pulse/Iris/Logic/IrisDesc/*`
- Browser runtime policy를 분리했다.
  - recipe requirement color/text: `Iris/UI/Browser/IrisRequirementPolicy.lua`
  - MapIcon coordinates: `IrisConfig.MAP_ICON_BUTTON`
- build surface를 정리했다.
  - root `Iris/build` Python entrypoints documented
  - pipeline helpers / one-shots / tests / versioned data moved to explicit subdirectories
- item-selection regression을 수정했다.
  - `Iris/Logic/IrisDesc/*.lua` BOM 제거
  - `IrisAPI.lua` generator load attempt guard 추가
  - targeted in-game smoke에서 error accumulation fixed 확인
- validation/package gate를 통과했다.
  - `319 tests / OK`
  - `test_require_render.py`: PASS
  - `quality_gates.py`: PASS
...
- current runtime/build contract를 유지한다.
  - public require path stability
  - Lua-only runtime
  - protected-call boundary centralized policy
  - BOM-free runtime Lua compatibility
- vanilla-first MVP posture를 유지한다.

### Next

- release가 필요하면 별도 full release manual QA checklist를 연다.
- resolver legacy compatibility mapping cleanup 여부는 별도 opening decision으로 판단한다.
- source expansion / external-mod normalization은 current MVP 이후 structure-only 원칙으로만 연다.

### Hold

- targeted smoke를 full release QA pass로 읽는 것
- runtime JSON parser 도입
- old `Pulse/Iris/Logic/IrisDesc/*` wrapper 즉시 삭제
- `ready_for_release` 선언
- Pulse/Iris 또는 Iris/other-spoke dependency 추가
- runtime-side compose/rewrite
- facts 슬롯 확장
...
# 32. 2026-05-08 Addendum — Iris final refactoring roadmap v1.4 implementation closeout

## 이번 addendum의 상태 해석

- 이번 addendum은 `docs/Iris/iris-refactoring-final-roadmap-v1.md`가 Planning 상태에서 implementation closeout 상태로 전환됐음을 기록한다.
- closeout state는 `implemented and closed at static-test plus console-validation level`이다.
- 이 addendum은 packaging, commit, release note, Workshop 배포, `ready_for_release` 선언이 아니다.

## Iris

### Done

- Phase 1부터 Phase 5-9까지 final roadmap implementation scope를 닫았다.
- closeout authority를 `Iris/_docs/refactor/iris_refactoring_final_roadmap_closeout.md`로 고정했다.
- Phase 5-9 module bootstrap cleanup을 마지막 구현 항목으로 닫았다.
  - `Iris/Util/IrisModuleBootstrap.lua` 도입
  - repeated logger/safeRequire bootstrap boilerplate 공통화
  - `IrisMain.lua` release-visible print fallback 유지
  - `IrisLogger.lua`와 helper 사이 require cycle 회피
- 최신 전체 정적 검증을 `376 tests / OK`로 기록했다.
- 최신 KO runtime smoke를 통과로 기록했다.
...
- packaging / release notes / commit은 별도 scope로 열지 결정한다.
- packaging을 열 경우 `Iris/tools/package_iris.ps1 -Clean -Zip`로 package folder, manifest, zip을 생성하고 산출물 검사를 별도로 수행한다.
- commit을 열 경우 dirty working tree 때문에 의도한 Iris refactor 파일만 stage한다.

### Hold

- roadmap closeout을 `ready_for_release`로 해석하는 것
- roadmap closeout을 Workshop 배포 완료로 해석하는 것
- targeted KO runtime smoke를 full release QA pass로 확대 해석하는 것
- dirty working tree에서 `git add .`로 전체 변경을 한 번에 묶는 것

---

# 33. 2026-05-12 Addendum — Iris refactoring roadmap v4.1 implementation closeout

## 이번 addendum의 상태 해석

- 이번 addendum은 `C:/Users/MW/Downloads/1.txt`의 `최종 리팩토링 로드맵 v4.1`이 implementation closeout 상태로 전환됐음을 기록한다.
- closeout state는 `implemented and closed at static-test plus workspace-copy in-game console-validation level`이다.
- 이 addendum은 P1/P2 정책 결정, packaging, commit, release note, Workshop 배포, `ready_for_release` 선언이 아니다.

## Iris

### Done

- Pre-Gate remeasurement를 `Iris/_docs/refactor/refactor_baseline_20260512.md`에 기록했다.
- T0-A/T0-B/T0-C를 닫았다.
  - Browser theme constants extracted
  - `.tmp/iris_backup_*` ignore/cached backup cleanup
  - bootstrap release log cleanup
- PG-7W/T1-A/T1-B 1단계를 닫았다.
  - fallback resolver checklist 작성
  - translation fallback resolver centralization
  - generic Java/Kahlua object access helper extraction
- T2-A/T2-B/T2-C를 닫았다.
  - item accessor helper extraction
  - `IrisUseCaseDescriptions.lua` requirements lookup split
  - no-op API deprecation markers and consumer note
- T3-C를 사용자의 실제 validation path 기준으로 닫았다.
  - active workspace runtime monolith removed
  - `layer3_renderer.lua` chunk manifest path used as runtime source
  - validation/closeout tooling updated to chunks-only runtime contract
  - `IrisContextMenuTextureCompat.lua` guards the observed context-menu null texture regression without adding an external mod dependency
- latest validation read:
  - `380 tests / OK`
  - `test_require_render.py`: PASS
  - `quality_gates.py`: PASS
  - `Console.txt` last write time `2026-05-12 20:42:50`
  ...
  - context-menu regression markers: 0 matches

### Next

- Open packaging/release-note/commit as a separate scoped task if needed.
- Open P2 only as a policy round that first decides `engine/ui/data/compat` protected-call recovery/log/fallback behavior.
- Open P1 only as a separate build-tooling manifest round after deciding manifest ownership and script grouping.

### Hold

- Reading P1/P2 as failed v4.1 implementation items.
- Reintroducing monolith/chunk dual runtime loading.
- Treating package-only exclusion as sufficient for the user's workspace-copy validation path.
- Declaring full release QA, deployed closeout, Workshop publication, or `ready_for_release` from this roadmap closeout.
