...
* Consolidated Iris core refactor Changes 1–8과 Change 9 sealed candidate를 완료했다.
  ...
  * generation-10 manifest/ceiling은 historical corpus JSON/ZIP을 포함한 required 75, reserved future 0, lifecycle 전부 sealed, mandatory unvalidated axis 0으로 봉인한다. Generation 8의 정상 git stderr를 PowerShell 5.1이 오류로 올린 operational 시도와, CleanCheckout PASS 뒤 stale ceiling binding hash가 발견된 generation 9을 supersede한다. Generation 10은 native exit code와 six binding Git-blob SHA-256 1:1 guard를 사용한다. Sealed candidate 자체는 `implemented_only`이며 Change 9 최종 완료 권위는 exact commit CleanCheckout 결과를 결속한 post-seal `final_evidence_binding_report.json.status=complete`다.
