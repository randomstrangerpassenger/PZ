...
> Phase 3 attempt-0008 bundle: sha256:A8F69843BB10325A60215FC06478DA48B2F3A02E0E64B82E9EA1FEB9318DFCDE / preflight FAIL / Windows non-extended path omitted 261-character review evidence / terminal report sha256:490D1B35C3DDFFFDFA9A4AF1A8BEAF0243042BC121C638F1FC69E88BC5706516 / byte-preserved
> Execution entry status: blocked until attempt-0009-entry receives fresh external reviews with Critical=0 and Important=0
...
* Iris/build/description/v2/owner_inputs/dvf_3_3_registry_authority_canonical_closure/focused_test_attestations/current_session_focused_test_execution_attestation.json
* Iris/build/description/v2/owner_inputs/dvf_3_3_registry_authority_canonical_closure/focused_test_attestations/current_session_focused_test_execution_failure_record.json (conditional; FAIL only, mutually exclusive with the PASS attestation)
...
* The exact focused-test command is directly observed by the designated separate Codex reviewer. The designation must bind the current cycle_id and attempt_id, list focused_test_execution_attestation as an exact assigned scope, set focused_test_execution_attestation_eligible=true, and exclude every implementation/test-target author from the reviewer identity. Only after that command exits 0 with the complete focused inventory and zero failure/error/skip counts may that reviewer author the reserved focused_test_attestations PASS input. The test target and closure runner cannot author the attestation. Implementation validates the full reviewer designation, current HEAD/test/inventory hashes, Entry/review bundle, exact command/result counts, and authoring chronology, then copies the attestation byte-for-byte before any WP write. To avoid self-reference, the bound Entry projection excludes only the attestation's own allowed git-status row; every other Entry field and row remains hash-bearing, and fresh Entry must still PASS after the attestation exists. A missing, self-authored, stale, or drifted attestation blocks implementation. If the command exits nonzero, the reviewer authors the mutually exclusive FAIL record instead, records any unavailable raw transcript honestly rather than fabricating a digest, and the attempt stops without implementation; retry requires a new attempt_id, preserved attempt evidence/owner inputs/FAIL record, a new checkpoint, and reapproval.
...
* A pre-adoption retry is legal only with a new attempt_id and new output root while protected_mutation_count=0, live_gate_adopted=false, and top_docs_applied=false. The registration lists every predecessor attempt, its preserved failure/archive path, and the canonical tree SHA-256 of that archive. For every post-separation attempts/<attempt_id> predecessor it also binds the exact superseded_owner_inputs/<attempt_id> path and canonical tree SHA-256. Preflight and Entry recompute both complete archive sets, every tree hash, and the predecessor terminal preflight report or write-once preflight exception record. Each post-separation owner-input archive must retain the checkpoint, registration, owner decision, reviewer designation, and plan approval; a materialized review attempt must also retain all three original review sources. For the focused-attestation contract introduced at attempt 21, any implementation execution trace requires both phase4/focused_test_execution_attestation.json and its byte-identical superseded_owner_inputs source; deleting either side and recomputing archive hashes is invalid. A focused-test FAIL before implementation instead requires its Reviewer-authored FAIL record, exact failed attempt evidence tree, exact owner-input tree, and base-commit code/test blobs; it can never be replaced by a same-attempt PASS attestation. A blocker-zero predecessor superseded before WP execution must retain its Entry, focused-test, implementation, or anchored command-sequence failure record as applicable. This protects the actual owner/reviewer inputs and separately captured failures as well as the attempt evidence. The tool must not delete, truncate, replace, rename-away, or rewrite a predecessor failure record.
* On Windows, archive enumeration, existence checks, byte reads, and canonical tree hashing must use resolved extended-length filesystem paths while retaining repo-relative logical names in the hash rows. A file beyond the legacy MAX_PATH boundary may not be treated as absent or omitted from a predecessor tree hash.
...
* predecessor_extended_length_file_hash_omission_count = 0
...
3. Focused test before implementation:
   ...
   The designated separate Codex reviewer directly observes this exact command and authors only the reserved focused-test execution attestation after a complete exit-0 run:

       [STOP — EXTERNAL FOCUSED-TEST EXECUTION ATTESTATION REQUIRED; THIS IS NOT A SHELL COMMAND]
       The designated reviewer authors only Iris/build/description/v2/owner_inputs/dvf_3_3_registry_authority_canonical_closure/focused_test_attestations/current_session_focused_test_execution_attestation.json.
       [RESUME ONLY AFTER THE ATTESTATION BINDS THE CURRENT HEAD, TEST/INVENTORY, ENTRY/REVIEW BUNDLE, EXACT COMMAND, EXIT CODE, COUNTS, OUTPUT HASH, AND CHRONOLOGY]

   A nonzero focused-test exit does not resume this attempt. The designated reviewer authors only the mutually exclusive current_session_focused_test_execution_failure_record.json, preserves any raw-output unavailability explicitly, archives the failed attempt and its owner inputs, and returns to step 1 with a new attempt_id, checkpoint, and approval after the fix.
