...
2026-08-31. 상태: **partial — 승인된 content와 runtime/격리 패키지 반영 및 필수 자동 검증 완료; 최종 pkg2/Iris 패키지의 식품류 인게임 표시·조작은 사용자 정상 확인, 그 밖의 대표 관찰 범위는 미보고**. 근거 있는 source review_hold 273개는 보존하며, 그 수 자체를 partial의 사유로 삼지 않는다.
...
관찰자는 사용자다. 설치 안내 뒤 사용자가 **`C:/Users/MW/PZ-U/pkg2/Iris` 폴더를 설치하고 식품류를 살펴봤다**고 확인했다. 따라서 아래 결과는 최종 패키지의 식품류 관찰에 연결한다. ZIP에서 직접 추출했다고 바꾸어 적거나 자동 검사·에이전트의 화면 관찰로 기록하지 않는다.

- 기본 아이템 툴팁에서 Alt 표시와 아이템 우클릭 Iris 상세 정보가 정상 작동한다.
- KO/EN의 긴 문장이 잘리거나 화면 밖으로 나가지 않는다.
- Recipe 표시와 Menu 전환이 정상 작동한다.

보고한 식품류의 화면·조작 범위는 정상으로 기록한다. exact item 목록과 게임 버전은 미제공이며, 사과 등 안내 예시를 실제 관찰한 item으로 추정하지 않는다. 설치 대상은 위 사용자 보고로 귀속하고 실제 게임 폴더를 탐색하거나 설치 hash를 새로 검사하지 않는다. 식품류의 보고를 계획 Change 6의 복수 major category 전체 관찰이나 모든 설명의 유용성·질문별 답 승인으로 확대하지 않는다. 인게임 검증은 사용자가 진행하며, 이번 응답에서 별도 재시험·추가 자료를 요구하지 않는다. 이번 반영은 기존 closeout의 관찰 기록만 갱신하며 제품 변경, 자동 테스트 재실행, 별도 proof artifact는 없다.
