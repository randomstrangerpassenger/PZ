...
uv run --project <measurement-tool-checkout> python -B <measurement-tool-checkout>\Iris\validation\test_workflow_consolidation\validate_measurement_comparability.py --base-repository <s-base-checkout> --terminal-repository <s-terminal-checkout> --accepted-session <external-paired-session-root>\session-receipt.json --accepted-schedule <external-paired-session-root>\accepted-session-schedule.json --family-ledger <terminal-family-disposition-ledger> --protocol-qualification-receipt <external-qualification-root>\qualification-receipt.json --measurement-contract <measurement-contract> --tooling-manifest <measurement-tooling-manifest> --touch-surface <declared-round-touch-surface> --contract-map <terminal-contract-preservation-map> --output <external-result-root>\measurement-comparability.json
```
