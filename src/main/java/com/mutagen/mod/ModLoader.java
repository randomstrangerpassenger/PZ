...
public class ModLoader {
    ...
    private ModLoader() {
        ...
        System.out.println("[Mutagen/ModLoader] Mods directory: " + modsDirectory.toAbsolutePath());
    }
    ...
    public void discoverMods() {
        if (discoveryComplete) {
            System.out.println("[Mutagen/ModLoader] Discovery already complete");
            return;
        }

        System.out.println("[Mutagen/ModLoader] ═══════════════════════════════════════");
        System.out.println("[Mutagen/ModLoader] Discovering mods...");

        // mods 폴더 생성 (없으면)
        try {
            if (!Files.exists(modsDirectory)) {
                Files.createDirectories(modsDirectory);
                System.out.println("[Mutagen/ModLoader] Created mods directory");
            }
        } catch (IOException e) {
            System.err.println("[Mutagen/ModLoader] Failed to create mods directory: " + e.getMessage());
            return;
        }
        ...
        try (DirectoryStream<Path> stream = Files.newDirectoryStream(modsDirectory, "*.jar")) {
            for (Path jarPath : stream) {
                try {
                    ...
                    System.err.println("[Mutagen/ModLoader] Failed to load mod: " + jarPath.getFileName());
                    e.printStackTrace();
                }
            }
        } catch (IOException e) {
            System.err.println("[Mutagen/ModLoader] Failed to scan mods directory: " + e.getMessage());
        }

        discoveryComplete = true;
        System.out.println("[Mutagen/ModLoader] Discovered " + mods.size() + " mod(s)");
        System.out.println("[Mutagen/ModLoader] ═══════════════════════════════════════");
    }
    ...
    private void discoverMod(Path jarPath) throws Exception {
        System.out.println("[Mutagen/ModLoader] Scanning: " + jarPath.getFileName());

        try (JarFile jar = new JarFile(jarPath.toFile())) {
            // mutagen.mod.json 찾기
            JarEntry metadataEntry = jar.getJarEntry("mutagen.mod.json");

            if (metadataEntry == null) {
                System.out.println("[Mutagen/ModLoader]   - No mutagen.mod.json found, skipping");
                return;
            }
            ...
            if (metadata.getId() == null || metadata.getId().isEmpty()) {
                System.err.println("[Mutagen/ModLoader]   - Invalid mod: missing 'id' field");
                return;
            }

            // 중복 체크
            if (mods.containsKey(metadata.getId())) {
                System.err.println("[Mutagen/ModLoader]   - Duplicate mod ID: " + metadata.getId());
                return;
            }
            ...
            System.out.println("[Mutagen/ModLoader]   ✓ " + metadata);
        }
    }
    ...
    public void resolveDependencies() {
        System.out.println("[Mutagen/ModLoader] Resolving dependencies...");
        ...
        for (ModContainer container : mods.values()) {
            ...
            for (ModMetadata.Dependency dep : metadata.getDependencies()) {
                ...
                if (depMod == null) {
                    if (dep.isOptional()) {
                        System.out.println("[Mutagen/ModLoader]   - " + metadata.getId() +
        ...
        if (!errors.isEmpty()) {
            System.err.println("[Mutagen/ModLoader] Dependency errors:");
            for (String error : errors) {
                System.err.println("[Mutagen/ModLoader]   ✗ " + error);
            }
        }
        ...
        System.out.println("[Mutagen/ModLoader] Load order: " +
    ...
    private void topologicalSort(String modId, Set<String> visited, Set<String> visiting,
        ...
        if (visiting.contains(modId)) {
            System.err.println("[Mutagen/ModLoader] Circular dependency detected: " + modId);
            return;
        }
    ...
    public void registerMixins() {
        System.out.println("[Mutagen/ModLoader] Registering mod mixins...");
        ...
        for (ModContainer container : loadOrder) {
            ...
            for (String mixinConfig : mixinConfigs) {
                try {
                    ...
                    System.err.println("[Mutagen/ModLoader]   ✗ Failed to register " +
                            mixinConfig + ": " + e.getMessage());
                }
            }
    ...
    public void initializeMods() {
        if (initialized) {
            System.out.println("[Mutagen/ModLoader] Mods already initialized");
            return;
        }

        System.out.println("[Mutagen/ModLoader] ═══════════════════════════════════════");
        System.out.println("[Mutagen/ModLoader] Initializing mods...");
        ...
        for (ModContainer container : loadOrder) {
            try {
                ...
                failed++;
                System.err.println("[Mutagen/ModLoader] ✗ " + container.getId() + " failed:");
                e.printStackTrace();
            }
        }
        ...
        System.out.println("[Mutagen/ModLoader] ═══════════════════════════════════════");
        System.out.println("[Mutagen/ModLoader] Initialization complete: " +
