...
import com.mutagen.event.lifecycle.ModReloadEvent;
...
public class ModReloadManager {
    ...
    // 모드별 이벤트 구독 백업 (리로드용)
    private final Map<String, List<Object>> modEventSubscriptions = new ConcurrentHashMap<>();
    ...
    private boolean disableMod(String modId) {
        ...
        if (container == null) {
            System.err.println("[Mutagen/Reload] Mod not found: " + modId);
            return false;
        }

        if (disabledMods.contains(modId)) {
            System.out.println("[Mutagen/Reload] Mod already disabled: " + modId);
            return true;
        }

        try {
            System.out.println("[Mutagen/Reload] Disabling mod: " + modId);

            // 1. 모드의 onUnload 호출
            MutagenMod instance = container.getModInstance(MutagenMod.class);
            if (instance != null) {
                try {
                    instance.onUnload();
                } catch (Exception e) {
                    System.err.println("[Mutagen/Reload] Error in onUnload: " + e.getMessage());
                }
            }
            ...
            System.out.println("[Mutagen/Reload] Mod disabled: " + modId);
            return true;

        } catch (Exception e) {
            System.err.println("[Mutagen/Reload] Failed to disable mod: " + modId);
            e.printStackTrace();
            return false;
        }
    }
    ...
    private boolean enableMod(String modId) {
        ...
        if (!disabledMods.contains(modId)) {
            System.out.println("[Mutagen/Reload] Mod already enabled: " + modId);
            return true;
        }

        try {
            System.out.println("[Mutagen/Reload] Enabling mod: " + modId);
            ...
            if (instance != null) {
                try {
                    ...
                    System.err.println("[Mutagen/Reload] Error in onInitialize: " + e.getMessage());
                }
            }
            ...
            System.out.println("[Mutagen/Reload] Mod enabled: " + modId);
            return true;

        } catch (Exception e) {
            System.err.println("[Mutagen/Reload] Failed to enable mod: " + modId);
            e.printStackTrace();
            return false;
        }
    }
    ...
    public static void reloadAllConfigs() {
        ...
        System.out.println("[Mutagen/Reload] All configs reloaded");
    }
    ...
    private boolean reloadConfigInternal(String modId) {
        try {
            ...
            ConfigManager.reloadAll();
            System.out.println("[Mutagen/Reload] Config reloaded for: " + modId);
            return true;
        } catch (Exception e) {
            System.err.println("[Mutagen/Reload] Failed to reload config: " + modId);
            e.printStackTrace();
            return false;
        }
    }
    ...
    private boolean softReloadMod(String modId) {
        ...
        try {
            System.out.println("[Mutagen/Reload] Soft reloading mod: " + modId);
            ...
            System.out.println("[Mutagen/Reload] Mod soft reloaded: " + modId);
            return true;

        } catch (Exception e) {
            System.err.println("[Mutagen/Reload] Failed to soft reload mod: " + modId);
            e.printStackTrace();
            return false;
        }
    }
    ...
    private boolean hotSwapMod(String modId, File newJarFile) {
        if (!newJarFile.exists() || !newJarFile.getName().endsWith(".jar")) {
            System.err.println("[Mutagen/Reload] Invalid JAR file: " + newJarFile);
            return false;
        }

        try {
            System.out.println("[Mutagen/Reload] Hot swapping mod: " + modId);
            System.out.println("[Mutagen/Reload] New JAR: " + newJarFile.getAbsolutePath());
            ...
            if (container != null) {
                ...
                if (entrypoint != null && !entrypoint.isEmpty()) {
                    try {
                        ...
                        if (newInstance instanceof MutagenMod) {
                            // 새 인스턴스로 교체 (리플렉션)
                            ...
                            System.out.println("[Mutagen/Reload] Loaded new mod instance");
                        }
                    } catch (ClassNotFoundException e) {
                        System.out.println("[Mutagen/Reload] Entrypoint not found in new JAR, using existing");
                    }
                }
            }
            ...
            System.out.println("[Mutagen/Reload] Hot swap complete (partial - JVM limitations apply)");
            return true;

        } catch (Exception e) {
            System.err.println("[Mutagen/Reload] Hot swap failed: " + modId);
            e.printStackTrace();
            return false;
        }
    }
