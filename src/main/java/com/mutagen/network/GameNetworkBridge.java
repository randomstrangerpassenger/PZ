...
import com.mutagen.access.AccessWidener;
...
public class GameNetworkBridge {
    ...
    // Mutagen 커스텀 패킷 채널 ID
    private static final short MUTAGEN_PACKET_ID = (short) 0x4D55; // "MU" in hex
    ...
    public void initialize() {
        ...
        try {
            ...
            try {
                ...
                System.out.println("[Mutagen/Network] GameClient class loaded");
            } catch (ClassNotFoundException e) {
                System.out.println("[Mutagen/Network] GameClient class not found (may be server-only)");
            }

            try {
                gameServerClass = loader.loadClass("zombie.network.GameServer");
                System.out.println("[Mutagen/Network] GameServer class loaded");
            } catch (ClassNotFoundException e) {
                System.out.println("[Mutagen/Network] GameServer class not found (may be client-only)");
            }

            try {
                byteBufferWriterClass = loader.loadClass("zombie.core.network.ByteBufferWriter");
                System.out.println("[Mutagen/Network] ByteBufferWriter class loaded");
            } catch (ClassNotFoundException e) {
                System.out.println("[Mutagen/Network] ByteBufferWriter class not found");
            }

            try {
                udpConnectionClass = loader.loadClass("zombie.network.UdpConnection");
                System.out.println("[Mutagen/Network] UdpConnection class loaded");
            } catch (ClassNotFoundException e) {
                System.out.println("[Mutagen/Network] UdpConnection class not found");
            }

            initialized = true;
            System.out.println("[Mutagen/Network] Network bridge initialized");

        } catch (Exception e) {
            System.err.println("[Mutagen/Network] Failed to initialize network bridge: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    public boolean sendToServer(byte[] data) {
        ...
        if (gameClientClass == null) {
            if (debugMode) {
                System.err.println("[Mutagen/Network] Cannot send to server: GameClient not available");
            }
            return false;
        }
        ...
        try {
            ...
            // Mutagen 패킷 ID 쓰기
            writeShort(writer, MUTAGEN_PACKET_ID);
            ...
            if (sendMethod != null) {
                ...
                if (debugMode) {
                    System.out.println("[Mutagen/Network] Sent " + data.length + " bytes to server");
                }
                return true;
            }
            ...
            System.err.println("[Mutagen/Network] Error sending to server: " + e.getMessage());
            if (debugMode) {
                e.printStackTrace();
            }
            return false;
    ...
    public boolean sendToClient(Object connection, byte[] data) {
        ...
        if (gameServerClass == null) {
            if (debugMode) {
                System.err.println("[Mutagen/Network] Cannot send to client: GameServer not available");
            }
            return false;
        }
        ...
        try {
            ...
            if (connection != null && udpConnectionClass != null &&
                ...
                if (sendMethod != null) {
                    ...
                    if (debugMode) {
                        System.out.println("[Mutagen/Network] Sent " + data.length + " bytes to client");
                    }
                    return true;
                }
            }
            ...
            System.err.println("[Mutagen/Network] Error sending to client: " + e.getMessage());
            if (debugMode) {
                e.printStackTrace();
            }
            return false;
    ...
    public boolean sendToAll(byte[] data) {
        ...
        if (gameServerClass == null) {
            if (debugMode) {
                System.err.println("[Mutagen/Network] Cannot send to all: GameServer not available");
            }
            return false;
        }
        ...
        try {
            ...
            if (connections == null) {
                if (debugMode) {
                    System.out.println("[Mutagen/Network] No connections found, cannot broadcast");
                }
                return false;
            }
            ...
            if (connections instanceof Iterable<?>) {
                ...
                if (debugMode) {
                    System.out.println("[Mutagen/Network] Broadcast to " + sent + " clients");
                }
                return sent > 0;
            }
            ...
            System.err.println("[Mutagen/Network] Error broadcasting: " + e.getMessage());
            if (debugMode) {
                e.printStackTrace();
            }
            return false;
    ...
    public void handleReceived(ByteBuffer buffer, Object sender) {
        try {
            ...
            if (debugMode) {
                System.out.println("[Mutagen/Network] Received " + length + " bytes");
            }

        } catch (Exception e) {
            System.err.println("[Mutagen/Network] Error handling received packet: " + e.getMessage());
            if (debugMode) {
                e.printStackTrace();
            }
        }
    ...
    private boolean sendRawPacket(byte[] data, Object target) {
        ...
        if (debugMode) {
            System.out.println("[Mutagen/Network] Raw packet send not implemented (" +
                    data.length + " bytes)");
        }
        return false;
    }
    ...
    /**
     * Mutagen 패킷 ID 반환 (Mixin에서 패킷 타입 확인용)
     */
    public static short getMutagenPacketId() {
        return MUTAGEN_PACKET_ID;
    }
}
