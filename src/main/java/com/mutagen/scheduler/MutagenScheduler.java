 ...
 * <pre>
 * // 60틱(약 3초) 후 1회 실행
 * TaskHandle handle = MutagenScheduler.runLater(() -> {
 ...
 * </pre>
 */
public class MutagenScheduler {

    private static final MutagenScheduler INSTANCE = new MutagenScheduler();
    ...
    private final ExecutorService asyncExecutor = Executors.newCachedThreadPool(r -> {
        Thread t = new Thread(r, "Mutagen-Async-" + taskIdGenerator.incrementAndGet());
        t.setDaemon(true);
        return t;
    });
    ...
    private MutagenScheduler() {
    }

    public static MutagenScheduler getInstance() {
        return INSTANCE;
    }
    ...
    private TaskHandle scheduleOnce(Runnable task, long delayTicks, String name) {
        ...
        if (debug) {
            System.out.println("[Mutagen/Scheduler] Scheduled once: " + taskName +
    ...
    private TaskHandle scheduleRepeating(Runnable task, long periodTicks, long delayTicks, String name) {
        ...
        if (debug) {
            System.out.println("[Mutagen/Scheduler] Scheduled repeating: " + taskName +
    ...
    private TaskHandle executeAsync(Runnable task, String name) {
        ...
        asyncExecutor.submit(() -> {
            try {
                ...
                System.err.println("[Mutagen/Scheduler] Async task error: " + taskName);
                e.printStackTrace();
            }
        });

        if (debug) {
            System.out.println("[Mutagen/Scheduler] Submitted async: " + taskName);
        }
    ...
    private void processSyncQueue() {
        ...
        while ((task = syncQueue.poll()) != null && processed < maxPerTick) {
            try {
                ...
                System.err.println("[Mutagen/Scheduler] Sync task error");
                e.printStackTrace();
            }
        }
    }
